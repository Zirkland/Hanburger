...
public class ShaderCode : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        m = new Material(image.material);// 复制Image的原始材质（创建实例），避免修改共享材质影响其他卡片
        image.material = m;

        visual = GetComponentInParent<CardVisual>();

        string[] editions = new string[4];
        editions[0] = "REGULAR";       // 普通版
        editions[1] = "POLYCHROME";    // 彩色版（推测）
        editions[2] = "REGULAR";       // 普通版（重复增加概率）
        editions[3] = "NEGATIVE";      // 负片版（推测）

        //禁用当前材质中所有已启用的关键字（避免版本冲突）
        for (int i = 0; i < image.material.enabledKeywords.Length; i++)
        {
            image.material.DisableKeyword(image.material.enabledKeywords[i]);
        }
        //随机选择一个版本，并启用对应的shader关键字（触发该版本的视觉效果）
